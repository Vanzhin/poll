...
<!-- /// -->
     <AdminCreateQuestionConformityRichtAdd
...
export default {
  ...
  created(){
    if (this.operationEdit) {
      ...
      this.uniqueNumber = this.numberAnswers = this.questions.getQuestion.subtitles.length
      this.richtVariants = this.questions.getQuestion.variant
      this.answers = this.questions.getQuestion.subtitles.map((item, index) => 
        {
