<template>
   <div class="block">
    ...
    <div class="container">
      <div class="row">
        <div
          v-if="tickets.getTickets"
        >
          <div v-for="(ticket ) in tickets.getTickets" 
            :key="ticket.id"
          >
            <AdminItemTicket
              :ticket="ticket"
...
<script setup>
  ...
  function addTicket(){
    navigateTo(`/admin/categorys/${route.params.id}/test/${route.params.testId}/ticket/create`)
  }
  onMounted (async() => {
    if (tests.getTest){
    ...
    await tickets.getTicketsTestIdDb({
