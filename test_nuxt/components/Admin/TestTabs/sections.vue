<template>
  <div >
    ...
    <div class="container">
      <div class="row">
        <div
          v-if="sections.getSections"
        >
          <div v-for="(section ) in sections.getSections" 
            :key="section.id"
          >
            <AdminItemSection
              :section="section"
...
<script setup>
  ...
  function addSection(){
    navigateTo(`/admin/categorys/${route.params.id}/test/${route.params.testId}/section/create`)
  }
  onMounted (async() => {
    if (tests.getTest){
    ...
    await sections.getSectionTestIdDb({
